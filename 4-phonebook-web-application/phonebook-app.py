...
app.config['MYSQL_DATABASE_PASSWORD'] = 'Zach'
app.config['MYSQL_DATABASE_DB'] = 'ms_phonebook'
...
# Write a function named `init_todo_db` create phonebook table within ms_phonebook db, if it doesn't exist

def init_phonebook_db():
    phonebook_table = """
    CREATE TABLE IF NOT EXISTS ms_phonebook.phonebook(
...
def find_records():
    if request.method == 'POST':
        ...
        return render_template('index.html', persons_html=persons_app, keyword=keyword, show_result=True, developer_name='Vincenzo')
    else:
